...
if($_REQUEST['sub'])
{
$sel=sqlsrv_query($con, "select id from register where id='$id' ");
$arr=sqlsrv_fetch_array($sel, SQLSRV_FETCH_BOTH);

if($arr['id']!=$id)
  {
   if(sqlsrv_query($con, "insert into register values('$title','$fname','$lname','$gen','$id','$pass','$phone','$add','$city','$coun','$dob')"))
	   {
